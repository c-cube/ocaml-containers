...
module Make (H : HashedType) : S with type key = H.t = struct
  ...
  let rec power_two_larger ~than n =
    if n >= than then
      ...
      power_two_larger ~than (2 * n)

  let create i =
    let i = power_two_larger ~than:i 16 in
    { length = 0; arr = Arr (Array.make i Nil) }

  let empty () = create 16

  let rec reroot_rec_ t k =
    match t.arr with
    | Arr a -> k a
    | Set (i, v, t') ->
      reroot_rec_ t' (fun a ->
  ...
  let find t k =
    ...
    | Nil -> raise Not_found
    | Cons (k1, v1, l1) ->
      if H.equal k k1 then
        v1
      else (
        match l1 with
        | Nil -> raise Not_found
        | Cons (k2, v2, l2) ->
          if H.equal k k2 then
            v2
          else (
            match l2 with
            | Nil -> raise Not_found
            | Cons (k3, v3, l3) ->
              if H.equal k k3 then
                v3
              else (
                match l3 with
                | Nil -> raise Not_found
                | Cons (k4, v4, l4) ->
                  if H.equal k k4 then
                    v4
                  else
                    find_rec_ k l4
              )
  ...
  let replace t k v =
    ...
    match a.(i) with
    | Nil ->
      if t.length > Array.length a lsl 1 then (
        (* resize *)
    ...
    | Cons _ as l ->
      let l', is_new = replace_rec_ k v l in
      if is_new && t.length > Array.length a lsl 1 then (
  ...
  let merge ~f t1 t2 =
    ...
    let tbl =
      fold
        (fun tbl k v1 ->
          let comb = try `Both (v1, find t2 k) with Not_found -> `Left v1 in
          match f k comb with
          | None -> tbl
  ...
  let add_list init l = add_iter init (fun k -> List.iter k l)
  let of_list l = add_list (empty ()) l
  let to_list t = fold (fun acc k v -> (k, v) :: acc) [] t
  let to_iter t k = iter t (fun x y -> k (x, y))
  ...
  let stats t =
    ...
    let max_bucket_length =
      Array.fold_left (fun n b -> max n (buck_length_ b)) 0 a
    in
